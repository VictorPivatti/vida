...
export function renderMedicos() {
  ...
  chart('chartPlantaoBox', { type: 'bar', data: { labels: allDates.map(dk => { const d = new Date(dk + 'T12:00:00'); return d.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }); }), datasets: [{ label: 'Diurno', data: allDates.map(dk => dByDate[dk] || 0), backgroundColor: '#1357a6', borderRadius: 2 }, { label: 'Noturno', data: allDates.map(dk => nByDate[dk] || 0), backgroundColor: '#7b61c4', borderRadius: 2 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: tickColor(), usePointStyle: true } }, tooltip: { callbacks: { label: c => `${c.dataset.label}: ${c.parsed.y} atend.` } } }, scales: { x: { grid: { color: gridColor() }, ticks: { color: tickColor(), maxRotation: 60, font: { size: 8 } } }, y: { grid: { color: gridColor() }, ticks: { color: tickColor() }, title: { display: true, text: 'atendimentos/plantão', color: tickColor(), font: { size: 9 } }, beginAtZero: true } } } });
